...
from graphbrain_semsim.eval_tools.datasets.dataset_table import ANNOTATION_LABELS, N_HEADER_ROWS
from graphbrain_semsim.utils.general import load_json, save_json

VALID_LABELS: list[int] = [ANNOTATION_LABELS['conflict'], ANNOTATION_LABELS['no conflict']]
ANNOTATION_FILE_SUFFIX: str = "annotated"


def recreate_dataset_from_table(dataset_name: str, full_dataset_name: str):
    table_file_path: Path = DATA_DIR / f"{dataset_name}_{ANNOTATION_FILE_SUFFIX}.xlsx"
    dataset_file_path: Path = DATA_DIR / f"{dataset_name}_recreated.json"
    ...
    full_dataset: LemmaDataset = load_json(DATA_DIR / f"{full_dataset_name}.json", LemmaDataset)
...
def update_annotation_labels_from_table(dataset_name: str):
    table_file_path: Path = DATA_DIR / f"{dataset_name}_{ANNOTATION_FILE_SUFFIX}.xlsx"
    dataset_file_path: Path = DATA_DIR / f"{dataset_name}.json"
