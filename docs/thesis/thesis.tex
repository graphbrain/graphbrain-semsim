...
%A structure that is in line with most of the reviewed literature employs the following categories: \textit{Knowledge-based Semantic Similarity} (KB\gls{ss}), \textit{Distributional} or \textit{Corpus-based Semantic Similarity} (CB\gls{ss}), \textit{Deep Learning-} or \textit{Neural Network-based Semantic Similarity} (NN\gls{ss}) and hybrid approaches which combine techniques from multiple categories 
...
%
%\todo[inline]{
%arguments for \gls{ness} (instead of DL\gls{ss}):
...
\todo{term differentiation: \gls{ness-shpm} and (F/C)\gls{ness}}
...
The CD is the result of the filtering, sampling and labelling described above. The size of the Worldnews-\gls{sh}, BES, FBES and CD are listed in \cref{tab:dataset-descriptions} for comparison. If applicable the the number of unique lemmas as well as the number and percentage of edges which are labelled as an expression of conflict and of those which are not are also noted.\todo{add examples? (in appendix?)}
...
It can be generally stated that the recall (\(r\)) of \gls{ness-shpm} in relation to the similarity threshold (\(r(t_s)\)) is strictly monotonically decreasing, since the set of points in embedding space that is inside of the similarity boundary consistently gets smaller with increasing threshold. \todo{maybe add ref. to earlier section}
...
%	\item For \gls{cness} with the AT option disabled, the gte model has a lower standard deviation of best F1-score than the e5 model in 3/3 (100\%) direct comparisons \\ (\texttt{semsim-ctx gte r-N-mean-best} vs \texttt{semsim-ctx e5 r-N-mean-best})
\end{enumerate}
...
%While the lemma-based \gls{fness} system is only able to match all or none of the edges in a PLES, the \gls{cness} system is potentially able to differentiate between "conflict" and "no conflict" given a PLES.
...
\\ \textbf{Supporting Observations:} \Cref{obs-itm:ness-better-without-AT}, \Cref{obs-itm:cness-lower-variation-with-AT}
