...
\todo{refer to the RQs more specifically? remove quant. / qual. distinction}
...
\todo{rename this, if no additional plots are added}

\paragraph{Evaluation Metrics vs. ST Plots}
These plots visualise the resulting evaluation scores for the different evaluation metrics in relation to different value for the similarity threshold. 

\Cref{fig:best-f1-per-pattern} shows the F1-score vs. the ST for the best performing evaluation runs for each conflict pattern. That means for every conflict pattern, this shows the evaluation run with the configuration that resulted in the highest best F1-score. For the \texttt{random} and \texttt{original} evaluation runs, there is obviously no configuration to choose from. For the semsim-fix based evaluation runs, the run with the best performing FNESS model is selected. For the semsim-ctx based evaluation runs, the run with the highest mean-best score regarding CNESS model, all-tokens option and reference edge set size is chosen. These evaluation runs correspond to those highlighted by bold letters in \cref{tab:quant-results}.

\Cref{fig:prec-rec-best-semsim} shows the the same evaluation runs as \cref{fig:best-f1-per-pattern} excluding the semsim-fix conflict pattern which is not lemma based. Instead of the F1-score this figure shows the scores of precision and recall vs. the ST. 

The plots in this section are selected because they were deemed to be most relevant for the observations made and discussed in \cref{sec:result-discussion}.
A more comprehensive visual comparison of the different evaluation configurations can be found in \cref{app-sec:eval-metric-vs-st-plots}.
